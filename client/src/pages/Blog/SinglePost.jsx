...
const SinglePost = () => {
  ...
  // Check if current user is the author (supports both ID- and name-based posts)
  const postAuthorId = post?.author?._id || post?.authorId || post?.author_id;
  const isAuthor = (
    isAuthenticated && user && post && (
      (postAuthorId && (user._id === postAuthorId || user.id === postAuthorId)) ||
      (typeof post.author === 'string' && user.name === post.author)
    )
  );
        ...
        {/* Article */}
          ...
          <div className="px-6 py-8 border-b border-gray-200 dark:border-gray-700">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-gray-900 leading-tight dark:text-white">
                {post.title}
              </h1>
              {isAuthor && (
            ...
            <div className="h-1 w-16 bg-accent rounded-full mt-4 mb-4" />
